...
function Complete() {
  ...
  const route = useRoute<RouteProp<LoggedInParamList>>(); //route안에 params가 들어있어서 전달받을 수 있음
  ...
  const onTakePhoto = useCallback(() => {
    return ImagePicker.openCamera({ //카메라가 켜짐
      includeBase64: true, // 미리보기 표시 기능
      includeExif: true, //카메라 드는 방향에 따라 조정해주는 역할
      cropping:true,
      saveToPhotos: true,
    })
      .then(onResponse)
